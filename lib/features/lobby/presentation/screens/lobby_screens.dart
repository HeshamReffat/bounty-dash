...
class _LobbyScreenState extends State<LobbyScreen> {
  ...
  String get _serverUrl {
    final raw = _hostLocally ? _ipCtrl.text.trim() :
    "https://bounty-dash-server-production.up.railway.app/"; //;
    if (raw.isEmpty) return 'ws://localhost:8080/ws';

    // If user pasted a full URL (ws://, wss://, http://, https://), adapt it.
    if (raw.contains('://')) {
      if (raw.startsWith('ws://') || raw.startsWith('wss://')) {
        return raw.endsWith('/ws')
            ? raw
      ...
      if (raw.startsWith('http://')) {
        ...
        return hostOnly.endsWith('/ws')
            ? hostOnly
            : hostOnly + (hostOnly.endsWith('/') ? 'ws' : '/ws');
      }
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _ActionButton(
                            ...
                            icon: Icons.computer,
                            loading: false,
                            color: _hostLocally?Color(0xFF00E5FF):
                            Colors.white24,
                            onTap: () {
                              setState(() {
                                _hostLocally = !_hostLocally;
                              });
                            },
                          ),
                          ...
                          _ActionButton(
                            ...
                            onTap: () async {
                              ...
                              await context.read<LobbyCubit>().connect(
                                _serverUrl,
                              );
      ...
      child: BlocListener<LobbyCubit, LobbyState>(
        listener: (context, state) {
          if (state is LobbyStarting) {
            ...
            final matched = starting.players
                .where((p) => p.id == starting.playerId)
                .toList();
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(8),
        border: isYou ? Border.all(color: Colors.white38) : null,
      ),
      child: Row(
        children: [
          Text(isRunner ? '🏃' : '🔦', style: const TextStyle(fontSize: 20)),
